- **Status**: draft
- **Approval**: pending
- **Task**: Xây adapter, dataloader, batch spec, và config migration layer cho `darwinSkill` để benchmark/runtime có thể chạy trên framework mới mà không quay lại CLI-first architecture.
...
- **Deliverables**:
  ...
  - add config loading layer that can:
    ...
    - hand resolved objects into Python API or thin CLI
  - add tests for:
...
- **Done Criteria**:
  ...
  - config loading is usable from Python and reusable by thin CLI wrappers
